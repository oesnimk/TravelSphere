    ...
    <section className="p-4 bg-white shadow-sm">
      ...
      <ul className="space-y-3">
        {nearbyAttractions.map((attraction) => (
          <li key={attraction.id} className="p-3 bg-gray-100 rounded-lg shadow-sm cursor-pointer transition-colors hover:bg-gray-200 flex items-center" onClick={() => handleAttractionClick(attraction.id)}>
            <img src={attraction.image} alt={attraction.name} className="w-16 h-16 object-cover rounded-md mr-4" />
